-- Give every due live-sync plan bounded service without adding another Cron or
-- queue. Attempts, rather than only successes, rotate the due kinds so a
-- repeatedly failing kind cannot starve another due kind.
...
begin
  ...
  if v_lease.run_id is not null and v_lease.locked_until > v_decision_at then
    insert into public.sync_runs (
      ...
      'skipped',
      coalesce(
        (select run.sync_kind
         from public.sync_runs as run
         where run.id = v_lease.run_id),
        'targeted'
      ),
      v_decision_at,
      v_decision_at,
  ...
  if v_lease.run_id is not null then
    ...
    set status = 'failed',
        ...
        error_message_safe =
          'The previous Sync worker did not finish before its lease expired.'
    where run.id = v_lease.run_id
  ...
  if v_lease.backoff_until is not null
     and v_lease.backoff_until > v_decision_at then
    return query select
  ...
  select coalesce(
    array_agg(
      ...
      order by candidate.live_priority,
  ...
  from (
    select
      match.external_id,
      match.kickoff_at,
      case when match.status = 'live' then 0 else 1 end as live_priority
  ...
  if p_force then
    ...
    select due.kind into v_kind
    from (
      select 'targeted'::text as kind, 1 as tie_priority
      where cardinality(v_fixture_ids) > 0
        and (
          v_lease.last_targeted_at is null
          or v_lease.last_targeted_at <= v_decision_at - interval '1 minute'
        )
      union all
      select 'catalog'::text, 2
      where v_lease.last_catalog_at is null
         or v_lease.last_catalog_at <= v_decision_at - interval '12 hours'
      union all
      select 'reconciliation'::text, 3
      where v_lease.last_reconciliation_at is null
         or v_lease.last_reconciliation_at <=
           v_decision_at - interval '6 hours'
    ) as due
    left join lateral (
      select run.started_at
      from public.sync_runs as run
      where run.provider = p_provider
        and run.sync_kind = due.kind
        and run.lease_generation is not null
      order by run.started_at desc, run.id desc
      limit 1
    ) as attempt on true
    order by attempt.started_at nulls first, due.tie_priority
    limit 1;
  ...
  set generation = v_lease.generation,
      ...
      last_forced_at = case
        when p_force then v_issued_at
        else lease.last_forced_at
      end,
...
comment on function public.claim_sports_sync(text, boolean) is
  'Fenced due planner: after lease/backoff/cooldown checks, least-recently attempted due kinds rotate within three eligible claims; targeted selects live fixtures before stale candidates and NOT_DUE persists nothing.';
