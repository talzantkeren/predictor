-- Reconciliation is keyed by the league discovered from its work item. Fail
-- before delegation when the item is absent, and bind the post-lock re-read to
-- that same league so the delegate can never run under a different league key.
...
comment on function public.reconcile_completed_league(uuid, integer, text) is
  'Fixed-system-admin reconciliation that validates and discovers one league, acquires its advisory key, re-verifies and locks the same league-match work item in canonical row order, and only then delegates the apply/dismiss transaction.';
