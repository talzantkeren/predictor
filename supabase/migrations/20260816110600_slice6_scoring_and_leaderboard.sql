-- Slice 6 results and standings: minimal system-admin identity, atomic
-- deterministic scoring, and an RLS-preserving competition-ranked view.
...
comment on table public.system_admins is
  'Server-managed system administrators. User roles have no direct table access.';
...
comment on function public.is_system_admin() is
  'Returns only whether the authenticated caller is a server-managed system administrator.';
...
comment on function public.score_match(
  ...
  'Atomically applies a system result and deterministically overwrites scoring for every league without taking a league row lock.';
...
with member_totals as (
  select
    ...
    count(prediction.id) filter (
      where prediction.is_correct_outcome is true
    )::bigint as correct_outcomes,
    count(prediction.id) filter (
      where prediction.is_exact is true
    )::bigint as exact_scores,
    count(prediction.id)::bigint as predictions_submitted
...
comment on view public.league_leaderboard is
  'RLS-preserving active-member standings using competition rank by points and correct outcomes only.';

comment on table public.predictions is
  'One current exact-score prediction per league, match, and active member with deterministic stored scoring metadata.';
comment on column public.predictions.points is
  'Stored deterministic scoring output. score_match overwrites it from the current result and league rule version.';
