...
כל invariant תלוי־זמן שנכתב למסד נדגם מ־`clock_timestamp()` **אחרי** רכישת
הנעילה שמסדרת אותו, ולא מזמן תחילת ה־transaction ולא מקריאה לא־נעולה שקדמה לה.
transaction שהתחיל לפני הגבול ואז המתין לנעילה חייב להכריע לפי הזמן שבו הוא
באמת מחזיק את השורה. `now()` נשאר מתאים רק להערכת חשיפה בקריאה (RLS), שבה
עקביות בתוך אותה שאילתה עדיפה על דיוק במילישניות.
...
לאחר השגת נעילת המשחק הפונקציה דוגמת את זמן מסד הנתונים ב־`clock_timestamp()`
ודוחה מעבר ל־`finished` כאשר הזמן שנדגם קטן מ־`kickoff_at`. ביטול לפני מועד
המשחק נשאר חוקי ואינו מוחק ניחושים.
...
מדיניות הקריאה של תחזיות בודקת `now()` של PostgreSQL, כדי שאותה שאילתה תראה
גבול חשיפה אחיד. מסלול הכתיבה נפרד: `save_prediction` נועלת את שורת המשחק ורק
אז דוגמת `clock_timestamp()`, כך שניחוש שהתחיל לפני ה־kickoff וחיכה לנעילה
נדחה. ה־Client אינו שולח `user_id` סמכותי; הוא נגזר מ־`auth.uid()` או מאומת מולו.
...
ה־latch מוכרע בתוך הכותב הנעול של ה־Sync, אחרי `for update` על שורת המשחק
ולפי `clock_timestamp()` שנדגם שם; wrapper אינו מחשב אותו מראש מקריאה
לא־נעולה. לכן ביטול שנצפה לפני ה־kickoff אך נכתב אחריו קובע latch, ו־
reactivation מאוחר אינו יכול לפתוח מחדש ניחושים שכבר נחשפו.
...
| --- | --- | --- |
| שינוי ניחוש אחרי נעילה | `clock_timestamp() < kickoff_at` שנדגם ב־RPC אחרי נעילת שורת המשחק, ו־RLS כהגנת עומק | שנייה לפני/בדיוק/שנייה אחרי, ובנוסף session אמיתי שמחזיק את השורה על פני ה־kickoff |
