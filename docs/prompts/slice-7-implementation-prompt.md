...
2. `pg_try_advisory_xact_lock` עם מפתח קבוע ומתועד (קבוע מספרי מוצהר בקוד
   ...
   אותה טרנזקציה.** אם לא הושגה — אין המתנה חוסמת ואין עיבוד של החלון. השאלה
   האם הניסיון הזה נרשם כשורת `skipped`/`concurrent_run` פתוחה לדיון: ראה
   "MATCH-06" למטה, וענה עליה לפני שאתה כותב קוד.
3. בדיקת due-window: האם קיימים משחקים בחלון שמצדיק קריאת ספק. עם provider
   `manual` התשובה תמיד שלילית ⇒ `skipped` עם קוד הסיבה
   `outside_due_window`.
4. כתיבת שורת `sync_runs` **סופית** (`skipped` עם קוד סיבה בטוח ומובחן, או
   `failed`), כולל `finished_at`. במסלול הנוכחי לא נוצרת שורת `running`
   לעולם.

> **MATCH-06 — נקודה פתוחה שדורשת את דעתך לפני קוד.**
>
> `docs/product.md` הוא המסמך הקנוני העליון, ו־MATCH-06 דורש ש"כל ריצת סנכרון
> מתועדת, וריצות מקבילות אינן מעבדות אותו חלון פעמיים". יש מתח בין המחצית
> הראשונה לבין בקשה שהפסידה בנעילה ולא עשתה דבר.
>
> **שתי הכרעות אפשריות:**
>
> א. **הגדרתית** — בקשה שנדחתה לפני תחילת עיבוד אינה "ריצת Sync", ולכן אינה
>    מחויבת בתיעוד ואינה כותבת שורה.
>
> ב. **תיעוד מלא** — המפסיד בנעילה כותב שורת `sync_runs` סופית,
>    `status=skipped`, עם קוד סיבה מובחן `concurrent_run` (לעומת
>    `outside_due_window`).
>
> **העמדה המוצעת היא ב'**, מהנימוקים הבאים: המחצית השנייה של MATCH-06 עוסקת
> במפורש בריצות מקבילות, ולכן הוצאת המקרה הזה מחובת התיעוד מרוקנת את הדרישה
> בדיוק במקום שבשבילו נכתבה; א' פותרת סתירה עם המסמך הקנוני העליון על ידי
> צמצום הגדרתו, וזה מהלך כבד משינוי ארכיטקטוני; וב' משמרת את הראיה היחידה
> לתחרות בפרודקשן. העלות היא שורת לוג append-only חסרת אינווריאנט, ולכן
> כתיבתה מחוץ להגנת הנעילה בטוחה.
>
> **אל תקבל את ב' כגזירה.** בדוק אותה מול הקוד והמסמכים ואמור מה דעתך לפני
> שאתה כותב שורת קוד. אם אתה מזהה בעיה — למשל bloat תחת retry אגרסיבי, קושי
> בהבחנה בין contention אמיתי לבין רעש, או פרשנות אחרת ל־MATCH-06 —
> הצג אותה ואת החלופה שאתה מציע. הכרעה שונה מ־ב' לגיטימית לחלוטין אם היא
> מנומקת; מה שאינו לגיטימי הוא לבחור בשקט בלי לומר.
>
> שני קודי הסיבה מובחנים בכל מקרה שבו נרשמת שורה, כדי שמסך הסטטוס יבדיל בין
> "אין מה לעשות" לבין "מישהו אחר רץ".
...
- **pgTAP (`supabase/tests/sync.test.sql`):**
  ...
  - **בדיקת מקביליות אמיתית**: שני sessions; הראשון מחזיק את הטרנזקציה עם
    ה־xact lock, השני קורא ל־`record_sync_attempt`. החלון לא מעובד פעמיים
    (MATCH-06), ואחרי commit של הראשון קריאה נוספת מצליחה — כלומר הנעילה אכן
    שוחררה ולא דלפה. מה בדיוק נבדק אצל השני — שורת `skipped`/`concurrent_run`
    או היעדר שורה — נגזר מהכרעת MATCH-06 שסוכמה; התאם את הבדיקה אליה.
  - קודי הסיבה מובחנים בכל שורה שנרשמת: `concurrent_run` מול
    `outside_due_window`.
...
- קריאות Cron חוזרות ומקבילות בטוחות: החלון אינו מעובד פעמיים, הטיפול
  בבקשה המקבילה תואם להכרעת MATCH-06 שסוכמה ומתועד בה, אין נעילה שדולפת
  מעבר לטרנזקציה, ואין שורת `sync_runs` שנשארת לא־סופית.
