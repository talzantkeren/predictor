...
2. `pg_try_advisory_xact_lock` עם מפתח קבוע ומתועד (קבוע מספרי מוצהר בקוד
   ...
   אותה טרנזקציה.** אם לא הושגה — אין המתנה חוסמת ואין עיבוד של החלון, אבל
   **הניסיון עדיין מתועד**: שורת `skipped` סופית עם קוד הסיבה
   `concurrent_run`. ראה "MATCH-06" למטה.
...
> **MATCH-06 — חובת תיעוד מלאה.** `docs/product.md` הוא המסמך הקנוני העליון,
> ו־MATCH-06 דורש ש"כל ריצת סנכרון מתועדת, וריצות מקבילות אינן מעבדות אותו
> חלון פעמיים". **אסור** לפתור זאת בהגדרה שלפיה בקשה שהפסידה בנעילה אינה
> "ריצה" — המחצית השנייה של הדרישה עוסקת במפורש בריצות מקבילות, וקביעה כזאת
> הייתה מרוקנת אותה ומוחקת את הראיה היחידה לתחרות בפרודקשן. שורת לוג
> append-only היא חסרת אינווריאנט, ולכן כתיבתה מחוץ להגנת הנעילה בטוחה. שני
> קודי הסיבה חייבים להיות מובחנים כדי שמסך הסטטוס יבדיל בין "אין מה לעשות"
> לבין "מישהו אחר רץ".
...
- **pgTAP (`supabase/tests/sync.test.sql`):**
  ...
  - **בדיקת מקביליות אמיתית**: שני sessions; הראשון מחזיק את הטרנזקציה עם
    ה־xact lock, השני קורא ל`record_sync_attempt` ומקבל `skipped` עם
    `concurrent_run` — שורה אחת בדיוק, והחלון לא עובד פעמיים (MATCH-06).
    אחרי commit של הראשון, קריאה נוספת מצליחה — כלומר הנעילה אכן שוחררה
    ולא דלפה.
  - שני קודי הסיבה מובחנים: `concurrent_run` מול `outside_due_window`.
...
- קריאות Cron חוזרות ומקבילות בטוחות: קריאה מקבילה מתועדת כ־`skipped`/
  `concurrent_run` ואינה מעבדת את החלון פעם שנייה (MATCH-06), אין נעילה
  שדולפת מעבר לטרנזקציה, ואין שורת `sync_runs` שנשארת לא־סופית.
